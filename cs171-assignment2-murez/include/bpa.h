#ifndef CS171_HW2_BPA_H
#define CS171_HW2_BPA_H
...
auto reconstruct(const std::vector<Vertex> &points, float radius) -> std::vector<Triangle>;
...
struct Grid {
    Grid(const std::vector<Vertex> &points, float radius)
            : cellSize(radius * 2) {
        lower = points.front().position;
        upper = points.front().position;
        ...
        for (const auto &p : points)
            cell(cellIndex(p.position)).push_back({p.position, p.normal});//cell(cellIndex(p.position)).push_back({p.position, p.normal});
    }
            ...
            for (auto yOff : {-1, 0, 1}) {
                for (auto zOff : {-1, 0, 1}) {
                    ...
                    for (auto &p : cell(index))
                        if (glm::length(p.pos - point) < cellSize * cellSize &&
                            std::find(begin(ignore), end(ignore), p.pos) == end(ignore))
                            result.push_back(&p);
                }
            }
        }
...
auto computeBallCenter(MeshFace f, float radius) -> std::optional<vec3> {
    ...
    const vec3 toCircumCircleCenter =
            (cross(abXac, ab) * dot(ac, ac) + cross(ac, abXac) * dot(ab, ab)) / (2 * dot(abXac, abXac));
...
auto ballIsEmpty(vec3 ballCenter, const std::vector<MeshPoint *> &points, float radius) -> bool {
    return !std::any_of(begin(points), end(points), [&](MeshPoint *p) {
        return glm::length(p->pos - ballCenter) < radius * radius - 1e-4f; // TODO epsilon
    });
}
...
auto findSeedTriangle(Grid &grid, float radius) -> std::optional<SeedResult> {
    for (auto &cell : grid.cells) {
        const auto avgNormal = normalize(
                std::accumulate(begin(cell), end(cell), vec3{}, [](vec3 acc, const MeshPoint &p) {
                    return acc + p.normal;
                }));
        for (auto &p1 : cell) {
            ...
            for (auto &p2 : neighborhood) {
                for (auto &p3 : neighborhood) {
                    if (p2 == p3) continue;
                    MeshFace f{{&p1, p2, p3}};
                    if (dot(f.normal(), avgNormal) <
                        0) // only accept triangles which's normal points into the same half-space as the average normal of this cell's points
                        continue;
...
auto join(MeshEdge *e_ij, MeshPoint *o_k, vec3 o_k_ballCenter, std::vector<MeshEdge *> &front,
          std::deque<MeshEdge> &edges) -> std::tuple<MeshEdge *, MeshEdge *> {
